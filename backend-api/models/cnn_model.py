...
class DeepfakeInference:
    ...
    def __init__(
        ...
        if model_path and os.path.exists(model_path):
            ...
            detected_model_name = checkpoint.get("model_name")
            if not detected_model_name:
                detected_model_name = self._infer_model_name_from_state_dict(state_dict)
        ...
        if checkpoint is not None:
            ...
            print(f"[OK] Model loaded from: {model_path} (backbone={self.model_name})")
        else:
    ...
    @staticmethod
    def _infer_model_name_from_state_dict(state_dict: dict) -> str | None:
        classifier_weight = state_dict.get("classifier.0.weight")
        if classifier_weight is None or len(classifier_weight.shape) != 2:
            return None
        ...
        return dim_to_model.get(feature_dim)
